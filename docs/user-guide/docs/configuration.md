...
| `tenant_retention_max_age` | `TOISE_TENANT_RETENTION_MAX_AGE` | — | *(empty)* | per-tenant retention bounds as `tenant:duration` pairs (e.g. `imagroupe:2160h`); an unlisted tenant keeps `retention_max_age`, a listed one is pruned to its own bound even when the global one is `0`. Storage becomes a per-tenant variable — see [storage sizing](operations/storage-sizing.md). Requires a restart to change. |
