...
func run(cfg config.Config, storeCfg store.Config, logger *slog.Logger) error {
	...
	if cfg.MCPStdio {
		...
		if serveErr := mcp.New(st.Graph, st.Store).SetCadence(st.Engine).SetAnnotations(st.Annotations).SetIdentityThreshold(cfg.IdentityThreshold).ServeStdio(ctx); serveErr != nil && !errors.Is(serveErr, context.Canceled) {
	...
	mcpRouter := newTenantRouter(reg, logger, func(st *registry.Stack) (http.Handler, error) {
		return mcp.New(st.Graph, st.Store).SetCadence(st.Engine).SetObserver(queryMetrics).SetAnnotations(st.Annotations).SetAuditor(auditor).SetIdentityThreshold(cfg.IdentityThreshold).HTTPHandler(), nil
	})
